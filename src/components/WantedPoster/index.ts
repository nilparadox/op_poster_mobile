...
class WantedPoster extends HTMLElement {
  ...
  async #waitForFonts() {
    ...
    try {
      ...
      console.warn('Font readiness wait failed.', error)
    }
  }
  ...
  async connectedCallback() {
    ...
    try {
      ...
      const wantedImageInfo = this.#applyMobileLayout(
        this.#wantedImage.setSize({
          width: rect.width,
          height: rect.height,
  ...
  >(wantedImageInfo: T): T {
    ...
    return {
      ...wantedImageInfo,
      namePosition: {
        ...wantedImageInfo.namePosition,
        y: wantedImageInfo.namePosition.y - 50,
        height: 56
      },
      bountyInfo: {
        ...wantedImageInfo.bountyInfo,
        y: wantedImageInfo.bountyInfo.y - 24,
        height: 52,
        fontSize: 30
      }
    }
